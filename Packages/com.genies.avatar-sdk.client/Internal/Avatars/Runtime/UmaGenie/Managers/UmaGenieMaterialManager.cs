...
{
    internal sealed class UmaGenieMaterialManager : IDisposable
    {
        ...
        public UmaGenieMaterialManager(UmaGenie genie, IDictionary<string, string> mappedUmaIdentifiers = null)
        {
            _genie = genie;
            ...
            {
                Debug.LogError($"[{nameof(UmaGenieMaterialManager)}] couldn't get the renderer component from the UMA genie");
                return;
            }
        ...
        public void AddMaterial(IGenieMaterial material)
        {
            if (material is null)
            {
                Debug.LogError($"[{nameof(UmaGenieMaterialManager)}] tried to add a null material");
                return;
            }
            ...
            {
                Debug.LogError($"[{nameof(UmaGenieMaterialManager)}] invalid genie material slot ID: {slotId ?? "[null]"}");
                return;
            }
            ...
            // refresh slot active state
            bool isSlotActive = TryGetMaterialIndexInRenderer(slot.Identifier, out _);
            slot.SetActive(isSlotActive);
        ...
        public void RemoveMaterial(IGenieMaterial material)
        {
            ...
            // we need to rebuild so UMA resets the material instance. Except if the slot is currently inactive
            if (isSlotActive)
            {
        ...
        public void ClearMaterialSlot(string slotId)
        {
            ...
            // we need to rebuild so UMA resets the material instance. Except if the slot is was inactive
            if (isSlotActive)
            {
                _genie.SetUmaAvatarDirty();
            }
        ...
        public bool TryGetSharedMaterial(string slotId, out Material material)
        {
            ...
            if (
                TryGetSlot(slotId, out Slot slot, createIfNotFound: false) &&
                TryGetMaterialIndexInRenderer(slot.Identifier, out int materialIndex) &&
                materialIndex >= 0 && materialIndex < _materials.Count)
            {
        ...
        public void ApplyMaterials(bool forced = false)
        {
            ...
            {
                ApplyMaterialInSlot(slot, forced);
            }
        ...
        public void RefreshSlotActiveStates()
        {
            foreach (Slot slot in _slots.Values)
            {
                // if we can find the material index in the renderer then the slot is active
                bool isActive = TryGetMaterialIndexInRenderer(slot.Identifier, out _);
        ...
        private void ApplyMaterialInSlot(Slot slot, bool forced = false)
        {
            ...
            // only apply the material if it is found on the renderer
            if (!TryGetMaterialIndexInRenderer(slot.Identifier, out int index) || index < 0 || index >= _materials.Count)
            {
                return;
            }

            // notify the material that it is being applied (it may want to copy some configuration from the current material)
            Material previousMaterial = _materials[index];
            ...
            if (materialToApply is null || materialToApply == previousMaterial)
            {
                return;
            }

            // if it uses the same shader than the previously applied material, just update it
            if (materialToApply.shader == previousMaterial.shader)
            {
                previousMaterial.CopyPropertiesFromMaterial(materialToApply);
                previousMaterial.name = materialToApply.name;
                return;
            }
            ...
            Object.Destroy(previousMaterial);
            _materials[index] = new Material(materialToApply);
        }
        ...
        private bool TryGetMaterialIndexInRenderer(string identifier, out int index)
        {
            index = -1;

            // try to look for a UMA material whose name contains the identifier string
            foreach (UMAData.GeneratedMaterial generatedMaterial in _genie.Avatar.umaData.generatedMaterials.materials)
            {
                if (!generatedMaterial.umaMaterial.name.Contains(identifier))
                {
                    continue;
                }

                index = generatedMaterial.materialIndex;
                return true;
            }
