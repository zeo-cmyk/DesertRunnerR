...
namespace VContainer
{
    public interface IObjectResolver : IDisposable
    {
        ...
        Func<Type, object> FallbackResolveType { get; set; }
    ...
    public sealed class ScopedContainer : IScopedObjectResolver
    {
        ...
        public Func<Type, object> FallbackResolveType { get; set; }
        ...
        public object Resolve(Type type)
        {
            try
            {
                return Resolve(FindRegistration(type));
            }
            catch
            {
                var fallback = FallbackResolveType?.Invoke(type);
                if (fallback != null)
                {
                    return fallback;
                }
