...
namespace UMA
{
...
#endif
	{
		...
		{
			return new UMATransform() { hash = hash, name = name, parent = parent, parentName = parentName, position = position, rotation = rotation, scale = scale };
		}
...
#endif
	{
		...
		public unsafe void Execute(int index)
		{
            ...
            if (Deltas[index].sqrMagnitude > 0.0001f)
            {
                Results[0] = 1;
            }
        }
...
#endif
	{
		...
		public static bool isAllZero(Vector3[] deltas)
		{
            ...
            NativeArray<int> results = new NativeArray<int>(1, Allocator.TempJob);
			results[0] = 0;

			unsafe
			{
				fixed (Vector3* deltaPtr = deltas)
				{
					IsAllZeroJob job = new IsAllZeroJob
					{
						Deltas = deltaPtr,
						Results = results
					};

					JobHandle handle = job.Schedule(deltas.Length, 64);
					handle.Complete();
				}
			}

			bool isAllZero = results[0] == 0;
...
#endif
	{
		...
		{
			shapeName = null;
			foreach (UMABlendFrame frame in frames)
            {
                frame.Dispose();
            }
...
#endif
	{
...
#else
        private const int MAX_VERTEX_COUNT = 65534;
#endif
        private static List<Vector3> gVertices = new List<Vector3>(MAX_VERTEX_COUNT);
        private static Vector3[] gVerticesArray;
        private static List<Vector3> gNormals = new List<Vector3>(MAX_VERTEX_COUNT);
        private static Vector3[] gNormalsArray;
        private static List<Vector4> gTangents = new List<Vector4>(MAX_VERTEX_COUNT);
        private static Vector4[] gTangentsArray;
        private static List<Vector2> gUV = new List<Vector2>(MAX_VERTEX_COUNT);
        private static Vector2[] gUVArray;
        private static List<Vector2> gUV2 = new List<Vector2>(MAX_VERTEX_COUNT);
        private static Vector2[] gUV2Array;
        private static List<Vector2> gUV3 = new List<Vector2>(MAX_VERTEX_COUNT);
        private static Vector2[] gUV3Array;
        private static List<Vector2> gUV4 = new List<Vector2>(MAX_VERTEX_COUNT);
        private static Vector2[] gUV4Array;
        private static List<Color32> gColors32 = new List<Color32>(MAX_VERTEX_COUNT);
        private static Color32[] gColors32Array;
        private const int UNUSED_SUBMESH = -1;
        private static List<int>[] gSubmeshTris = {
        ...
        private static int[][] gSubmeshTriArrays;
        private static int[] gSubmeshTriIndices =
		{
			UNUSED_SUBMESH,
			UNUSED_SUBMESH,
			UNUSED_SUBMESH,
...
#region Blendshape
		...
		private void SetBoneWeightsFromMeshData(Mesh mesh)
		{
			...
			if (ManagedBoneWeights != null)
			{
				...
				{
					Debug.LogWarning("Error! Boneweights and BonesPerVertex is invalid. The slot must be regenerated.");
					return;
				}
