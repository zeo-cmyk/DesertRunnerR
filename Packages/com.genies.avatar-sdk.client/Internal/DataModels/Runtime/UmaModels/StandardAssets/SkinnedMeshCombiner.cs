...
namespace UMA
{
...
#endif
	{
		...
		private static void BuildBoneWeights(UMAMeshData data, NativeArray<BoneWeight1> dest, NativeArray<byte> destBonesPerVertex, int destIndex, int destBoneweightIndex, int count, int[] bones, Matrix4x4[] bindPoses, Dictionary<int, BoneIndexEntry> bonesCollection, List<Matrix4x4> bindPosesList, List<int> bonesList)
		{
			...
			unsafe
			{
                fixed (byte* sourceBones = data.ManagedBonesPerVertex)
                {
                    fixed (BoneWeight1* sourceWeights = data.ManagedBoneWeights)
                    {
				// Use unsafe code to manipulate the arrays without safety checks
				byte* destBonesPtr = (byte*)NativeArrayUnsafeUtility.GetUnsafePtr(destBonesPerVertex);
				BoneWeight1* destWeightsPtr = (BoneWeight1*)NativeArrayUnsafeUtility.GetUnsafePtr(dest);

				// Copy data manually without safety checks
				UnsafeUtility.MemCpy(destBonesPtr + destIndex, sourceBones, data.ManagedBonesPerVertex.Length);
				UnsafeUtility.MemCpy(destWeightsPtr + destBoneweightIndex, sourceWeights, data.ManagedBoneWeights.Length * UnsafeUtility.SizeOf<BoneWeight1>());

				// Adjust the bone weights using the mapping
				for (int i = 0; i < data.ManagedBoneWeights.Length; i++)
				{
					BoneWeight1 b = new BoneWeight1 { boneIndex = boneMapping[sourceWeights[i].boneIndex], weight = sourceWeights[i].weight };
					destWeightsPtr[i + destBoneweightIndex] = b;
				}
                    }
